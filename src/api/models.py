...
from sqlalchemy import (
    Boolean, DateTime, Date, ForeignKey, Integer, String, Text, 
    DECIMAL, func, UniqueConstraint, CheckConstraint
)
...
class User(db.Model):
    ...
    def serialize(self):
     ...
     ### __repr__ METHOD ###
    def __repr__(self):
        return f"<User ID {self.user_id} | Username: {self.username} | Email: {self.email} | Name: {self.first_name} {self.last_name}>"
...
class Space(db.Model):
    ...
    owner_id:      Mapped[int]      = mapped_column( Integer, ForeignKey("user.user_id",                   ondelete="CASCADE"), 
                                                                                          nullable=False)               
    ...
    def serialize(self):
     ...
     ### __repr__ METHOD ###
    def __repr__(self):
        return f"<Space ID {self.space_id} | Title: {self.title} | Price per day: {self.price_per_day} | Capacity: {self.capacity}>"
...
class Booking(db.Model):
    ...
    guest_id:    Mapped[int]      = mapped_column( Integer,  ForeignKey("user.user_id",         ondelete="CASCADE"), 
                                                                               nullable=False)
    space_id:    Mapped[int]      = mapped_column( Integer,  ForeignKey("space.space_id",       ondelete="CASCADE"), 
                                                                               nullable=False)
    ...
    def serialize(self):
     ...
     ### __repr__ METHOD ###
    def __repr__(self):
        return f"<Booking ID:{self.booking_id} | Guest ID:{self.guest_id} | Space ID:{self.space_id} | Dates: from {self.check_in} to {self.check_out} | Status: {self.status}>"
...
class Payment(db.Model):
    ...
    booking_id:     Mapped[int]      = mapped_column( Integer,  ForeignKey("booking.booking_id",    ondelete="CASCADE"), 
    ...
    def serialize(self):
     ...
     ### __repr__ METHOD ###
    def __repr__(self):
        return f"<Payment ID {self.payment_id} | Booking ID:{self.booking_id} | Amount:{self.amount} {self.currency} | Payment method:{self.payment_method}>"
